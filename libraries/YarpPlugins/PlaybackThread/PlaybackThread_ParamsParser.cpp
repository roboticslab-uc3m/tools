...
bool PlaybackThread_ParamsParser::getParamValue(const std::string& paramName, std::string& paramValue) const
{
    if (paramName =="file")
    {
        paramValue = m_file;
    ...
    if (paramName =="timeIdx")
    {
        paramValue = std::to_string(m_timeIdx);
        return true;
    }
    if (paramName =="timeScale")
    {
        paramValue = std::to_string(m_timeScale);
        return true;
    }
    if (paramName =="mask")
    {
...
std::string PlaybackThread_ParamsParser::getConfiguration() const
{
